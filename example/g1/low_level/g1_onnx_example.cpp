...
class G1Example {
 ...
 public:
  ...
  void Control() {
    ...
    motor_command_tmp.kp.at(LeftAnklePitch) = 40.0;
    motor_command_tmp.kp.at(LeftAnkleRoll) = 40.0;
    motor_command_tmp.kd.at(LeftAnklePitch) = 1.0;
    motor_command_tmp.kd.at(LeftAnkleRoll) = 1.0;

    mode_pr_ = Mode::PR;
    double max_P = M_PI * 20.0 / 180.0;
    double max_R = M_PI * 0.0 / 180.0;
    double L_P_des = max_P * std::sin(2.0 * M_PI * time_);
    double L_R_des = max_R * std::sin(2.0 * M_PI * time_);
